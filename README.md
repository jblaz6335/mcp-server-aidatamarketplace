...
[![x402scan](https://img.shields.io/badge/x402scan-61_resources-00c2ff)](https://www.x402scan.com/server/8dd63536-ba25-40f4-b36a-7d05ed18d007)
...
Version 2.6.0 can complete the x402 challenge, authorization, retry, and settlement automatically. The buyer key stays inside the buyer's local MCP process; the marketplace never receives it. Use a dedicated low-balance Base wallet, not a primary wallet.
...
- **npm:** [`dopaminedesk-ai-data-marketplace-mcp`](https://www.npmjs.com/package/dopaminedesk-ai-data-marketplace-mcp), version 2.6.0
...
The production catalog contains **61 capability-audited billable tools** and **104 total public catalog entries**. The billable set consists of 54 live-source tools, 1 verified government-contract snapshot, 5 input-sensitive processors, and 1 verified payment-credit service. Of all public entries, 64 are operational and 40 are labeled preview-only fixtures. Every API response identifies its data mode, availability, billing state, and backing source.
...
The paid OpenAPI contract contains exactly the 61 billable products. The broader catalog endpoint retains 40 clearly labeled preview fixtures for evaluation, but those routes do not appear as purchasable OpenAPI products.
...
| :--- | :--- | :--- |
| `GET /api/v1/evm_block_number` | Current Base, Ethereum, or Arbitrum block height | **0.001 USDC** |
| `GET /api/v1/evm_block` | Live block header and bounded transaction hashes | **0.002 USDC** |
| `GET /api/v1/evm_transaction` | Normalized transaction lookup by hash | **0.004 USDC** |
| `GET /api/v1/evm_receipt` | Execution status, gas, and event logs | **0.003 USDC** |
| `GET /api/v1/evm_nonce` | Confirmed or pending account nonce | **0.0015 USDC** |
...
The server advertises Bazaar-compatible discovery schemas for all 61 products while keeping seller-owned OpenAPI, MCP, sitemap, and agent-purchase documents canonical. PayAI handles verification and settlement; USDC is delivered directly to the configured marketplace wallet.
